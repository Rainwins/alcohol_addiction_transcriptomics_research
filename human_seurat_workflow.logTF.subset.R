...
#Visualizing using TSNE/UMAP
pdf("human_umap.pdf")
human.combined <- RunUMAP(human.combined, dims = 1:14)
DimPlot(human.combined, reduction = "umap", label = TRUE)
dev.off()

#generate umap for each condition
human.combined |> 
  subset(subset = orig.ident == "control") |>
  RunUMAP(dims = 1:14) |>
  DimPlot(reduction = "umap") + ggtitle("Control")
